...
module.exports = function (projectHomePath){
    ...
    iteratorSrcFolder(srcFolder, projectHomePath, resultPage, projectHomePath)
}
// 遍历文件夹下面所有文件

function iteratorSrcFolder(sourcePath, outPath, resultPage){
    ...
    _.each(fs.readdirSync(sourcePath), function(fileName, index) {
        ...
        if (file.isFile() && fileName !== 'lib.txt') {
    ...
    console.log('creat [' + outPath + '/iframe.html]');
}
