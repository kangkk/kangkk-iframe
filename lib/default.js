...
module.exports = function (projectHomePath){
    ...
    var createHighlightFilePath = path.join(projectHomePath, 'highlight');
    var resultPage = path.relative(projectHomePath, 'dist/index.html');

    iteratorSrcFolder(srcFolder, createHighlightFilePath, resultPage, projectHomePath)
}
// 遍历文件夹下面所有文件

function iteratorSrcFolder(sourcePath, outPath, resultPage, projectHomePath){
    ...
    fs.writeFileSync(
        projectHomePath + '/iframe.html',
        ejs.render(
            creatHtmlTemplate(htmlTabString, htmlPreString, resultPage)
        ),
        'utf8'
    );
    console.log('creat [' + projectHomePath + '/iframe.html]');
}
...
function creatHtmlTemplate(tabs, pres, iframe) {
  return '<!DOCTYPE html>'
        ...
        + '  <link rel="stylesheet" href="https://iframe.kangkk.cn/lib/embed/embed-light.css"/>'
        ...
        + '  <script src="https://iframe.kangkk.cn/lib/embed/embed.js"></script>'
        ...
        + '<div id="full-embed">'
        + '<header>'
        + '  <h1><a href="https://iframe.kangkk.cn/" target="_blank">康健生活</a></h1>'
        + '  <div id="actions">'
        + '     <ul class="normalRes">'
        + tabs
        + '   </ul>'
